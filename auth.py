...
def show_login_page():
    ...
    with col2:
        ...
        <div style='text-align:center; margin-bottom:2rem;'>
            ...
            <div style='color:#8b949e;'>Empowering Farmers with Technology</div>
        </div>
    ...
    with col2:
        st.markdown(f"<div style='text-align:center; font-size:1.1rem; color:#8b949e; margin-bottom:1rem;'>{get_text('select_user_type')}</div>", unsafe_allow_html=True)
        ...
        <div style='text-align:center; margin:0.5rem 0;'>
            ...
            Selected: {"👨‍🌾 " + get_text('farmer') if user_type=="Farmer" else "🔧 " + get_text('admin')}
            </span>
        </div>
        """, unsafe_allow_html=True)
...
def _show_farmer_auth(lang):
    ...
    if mode == 'login':
        st.markdown(f"<div style='text-align:center; font-size:1.3rem; font-weight:600; color:#e6edf3; margin-bottom:1rem;'>👨‍🌾 {get_text('login')}</div>", unsafe_allow_html=True)
        ...
        st.markdown(f"<div style='text-align:center; font-size:1.3rem; font-weight:600; color:#e6edf3; margin-bottom:1rem;'>📝 {get_text('register')}</div>", unsafe_allow_html=True)

        name = st.text_input(f"👤 {get_text('name')}", placeholder="Enter your full name", key="reg_name")
        ...
        district = st.text_input(f"📍 {get_text('district')}", placeholder="Enter your district", key="reg_district")
        password = st.text_input(f"🔒 {get_text('password')}", type="password", key="reg_pass")
        confirm = st.text_input("🔒 Confirm Password / पासवर्ड दोहराएं", type="password", key="reg_confirm")
            ...
            if all([name, phone, state, district, password, confirm]):
                if len(phone.strip()) != 10 or not phone.strip().isdigit():
                    ...
                    try:
                        ...
                        fid = create_farmer(name.strip(), phone.strip(), state, district.strip(), ph)
...
def _show_admin_auth(lang):
    st.markdown(f"<div style='text-align:center; font-size:1.3rem; font-weight:600; color:#e6edf3; margin-bottom:1rem;'>🔧 {get_text('admin')} {get_text('login')}</div>", unsafe_allow_html=True)
    ...
    <div style='text-align:center; color:#8b949e; font-size:0.85rem;'>
        Default credentials: username = <b style='color:#39d353;'>admin</b> / password = <b style='color:#39d353;'>admin</b>
    </div>
    """, unsafe_allow_html=True)
