...
IRRIGATION_DATA = {
    # crop: {soil: {water_per_acre_liters, frequency_days, yield_quintal_per_acre}}
    "Wheat": {
    ...
    "Default": {
        "Default": {"water": 2500, "freq": 7, "yield": 15},
    }
}

def get_irrigation_plan(crop, soil, water_source, area):
    ...
    if water_source in ["Canal", "River", "Pond"]:
        ...
        efficiency_tip = "⚠️ Borewell usage - monitor groundwater levels. Consider scheduling at night."

    schedule = []
    for i in range(1, 6):
        schedule.append(f"Day {i * freq}: Apply {int(water_per_acre * 0.2):,} L/acre")

    return {
        "water_per_acre": f"{water_per_acre:,} L/acre",
        "total_water": f"{total_water:,} L",
        "frequency": f"Every {freq} days",
        "next_irrigation": f"In {freq} days",
        "estimated_yield": f"{total_yield:.1f} quintals",
        "yield_per_acre": f"{yield_per_acre} q/acre",
        "efficiency_tip": efficiency_tip,
        "schedule": schedule,
        "best_time": "Early morning (5-7 AM) or evening (6-8 PM)",
        "stages": _get_crop_stages(crop),
    }

def _get_crop_stages(crop):
    stages = {
        "Wheat": ["Sowing (Oct-Nov): 1 irrigation", "Tillering (Dec): 2nd irrigation", "Jointing (Jan): 3rd irrigation", "Heading (Feb): 4th irrigation", "Grain fill (Mar): 5th irrigation"],
        "Rice": ["Transplanting: Keep 5cm standing water", "Tillering: 3-5cm water", "Panicle initiation: 5cm water", "Flowering: Critical - no stress", "Ripening: Drain 2 weeks before harvest"],
        "Cotton": ["Germination: Light irrigation", "Vegetative: Every 10-12 days", "Flowering: Critical period, don't stress", "Boll development: Reduce frequency", "Maturity: Stop 4 weeks before harvest"],
        "Sugarcane": ["Germination: Light frequent", "Tillering: Increase frequency", "Grand growth: Maximum water", "Maturity: Reduce gradually", "Pre-harvest: Stop 4 weeks before"],
        "Default": ["Germination stage: Light irrigation", "Vegetative stage: Moderate irrigation", "Flowering stage: Critical period", "Grain/Fruit fill: Adequate water", "Maturity: Reduce water"],
    }
...
def get_crop_rotation(crop, soil):
    rule = ROTATION_RULES.get(crop, ROTATION_RULES["Default"])
    tip = ADDITIONAL_TIPS.get(soil, ADDITIONAL_TIPS["Default"])
    return {
        "next_crop": rule["next"],
        "reason": rule["reason"],
        "soil_impact": rule["soil_impact"],
        "soil_tip": tip,
        "fallow_option": "Consider 2-3 week fallow with green manure (Dhaincha/Sunhemp) between crops",
    }
...
HIGH_PRIORITY_KEYWORDS = [
    "flood", "drought", "dead", "disease", "pest", "locust", "no water",
    "crop failure", "loss", "damage", "emergency", "urgent", "dying",
    "contaminated", "poison", "toxic", "illegal", "bund broken", "canal broken",
    "बाढ़", "सूखा", "मर", "बीमारी", "कीट", "पानी नहीं", "फसल खराब", "नुकसान",
    "आपातकाल", "जरूरी", "जहर", "नाला टूटा"
]

MEDIUM_PRIORITY_KEYWORDS = [
    "delay", "not received", "pending", "subsidy", "payment", "application",
    "certificate", "repair", "maintenance", "request", "help", "issue", "problem",
    "विलंब", "नहीं मिला", "लंबित", "सब्सिडी", "भुगतान", "आवेदन", "मरम्मत"
]
